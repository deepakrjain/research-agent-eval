...
## 2026-07-22 — Independent LLM Judging (Phase 6)

**Decision:** Use a separate model family (Mixtral) for the judge than the agent (Llama 3).

**Why:** "Self-preference bias" is a known phenomenon where an LLM will rate text generated by itself or its own model family significantly higher than text from other models, because the linguistic patterns match its internal weights. By using Mixtral to judge Llama 3, we get a more objective evaluation.

**Decision:** Force the judge to output structured JSON (`EvaluationScore`).

**Why:** Same as the synthesizer — we need to extract a concrete integer score (1-5) and a boolean hallucination flag to calculate aggregate statistics across the 20-question benchmark. Parsing text like "I give this a 4 out of 5" using regex is brittle; JSON is deterministic.

**Decision:** Decouple the scoring script (`score.py`) from the benchmark runner (`runner.py`).

**Why:** If the judge API rate-limits or fails, we don't want to lose the expensive output generated by the agent. By running the agent first to generate a `.jsonl` file, and then passing that file to the scoring script, we can safely re-score the same run multiple times if we tweak the judge's prompt.
